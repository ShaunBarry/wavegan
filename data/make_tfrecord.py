if __name__  == '__main__':
  ...
  samps = samps / tf.reduce_max(tf.abs(samps))
  ...
  for i, start_idx in tqdm(enumerate(range(0, len(audio_fps), npershard))):
    ...
    for _audio_fp in audio_fps[start_idx:start_idx+npershard]:
      ...
      try:
        ...
        print(_slices.shape, _slices.dtype)
      except:
        print('Error')
        continue
      ...
      for j, _slice in enumerate(_slices):
        if np.abs(_slice).max() > 0.01:

          print('Example:',j)
          print(_slice.shape)
          example = tf.train.Example(features=tf.train.Features(feature={
            'id': tf.train.Feature(bytes_list=tf.train.BytesList(value=[str.encode(audio_id)])),
            'label': tf.train.Feature(bytes_list=tf.train.BytesList(value=[str.encode(audio_label)])),
            'slice': tf.train.Feature(int64_list=tf.train.Int64List(value=[j])),
            'samples': tf.train.Feature(float_list=tf.train.FloatList(value=_slice[:,np.newaxis]))
          }))
